...
/*
		...
		*/
		...
		select
		tcd.location, SUM(cast(tcd.new_deaths as int)) as TotalDeathCount, MAX(tcd.date) as date_
		...
		-- Cantidad más alta de infección por país, población y fecha
		...
		select continent, sum(new_deaths) as TotCountDeaths
		from tb_covid_deaths
		where continent is not null
		group by continent
		order by TotCountDeaths desc
